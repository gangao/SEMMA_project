...
1、forward基于score chi-squire统计量 ——实际上glm类中已经实现了该统计量检验即score_test
...
def forward_logit(step, indata, target, weight, varkeep, varleft, sle, vif_check, corr_check):
    ...
    for i, varname in enumerate(varleft):
        ...
        #2.5 新变量varname的单变量score统计量
        #    用旧的x变量列表进行回归，然后算score statistic(lm statistic)
        print('SCORE TEST :')
        x_var = ['const'] + varkeep #不包含新变量varname的老变量
        if weight != None:
            mod = sm.GLM(indata[target], indata[x_var], \
                         family=sm.families.Binomial(sm.families.links.logit()), \
                             freq_weights=indata[weight])
            ...
            mod = sm.GLM(indata[target], indata[x_var], \
                         family=sm.families.Binomial(sm.families.links.logit()), \
                             freq_weights=None)
        res = mod.fit()
        newvar = indata[varname].values
        st = mod.score_test(params_constrained=res.params, exog_extra=newvar)
        score_stats = st[0][0] #从回归结果中取出特定变量的score统计量相关结果
        pvalue = st[1][0] #从回归结果中取出特定变量的score统计量相关结果
        df_constraint = st[2] #从回归结果中取出特定变量的score统计量相关结果
        #插入forward_loop_record表
        forward_loop_record = forward_loop_record.append({'varno': i, 'varname': varname, 'score_stats': score_stats, \
    ...
    #3  跳出循环后，按score统计量从大到小排序，选最大的进行检验，然后选取变量或不选取
    forward_loop_record.sort_values(by=['score_stats'], ascending=False, inplace=True) #score统计量从大到小
...
logit_model_fit = logistic_stepwise(spector_data_df, 'GRADE', weight=None)   