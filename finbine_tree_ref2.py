...
def initial_range(special_value):
    ...
    left_interval = round(float(sorted(special_value,reverse=False)[0]))+1
    irange = [left_interval, float('inf')]
    return irange
...
def find_best_split(indata, var, target, vrange, brks, father_gini, weight, imprmt_threshold=0,
    ...
    if len(imprmt_list) == 0:
        ...
        best_imprmt = imprmt_list.sort(key=lambda x:x['gini_imprvmt'], reverse=True)[0] #按提高值排序
    return best_imprmt
...
def match_threshold(imprmt_list, imprmt_threshold, min_lf_obs, min_lf_obs_wt, min_lf_g_obs, min_lf_b_obs, min_lf_g_obs_wt, min_lf_b_obs_wt):
    ...
    for ele in imprmt_list:
        if (ele['gini'] > imprmt_threshold and ele['left_cnt'] > min_lf_obs and 
...
dict1 = {'gini':1, 'entropy':3}
dict2 = {'gini':2, 'entropy':2}
dict3 = {'gini':3, 'entropy':1}
...
spv_num = [-9999.1, -9999.2, -9999.3, -9999.4]

left_interval = round(float(sorted(spv_num,reverse=False)[0]))+1


import copy
list2 = copy.deepcopy(list1)
list2.sort(key=lambda x:x['entropy'], reverse=True)

def sort_the_best_imprmt(imprmt_list):
    imprmt_list.sort(key=ele())
