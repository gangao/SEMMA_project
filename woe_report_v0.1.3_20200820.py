...
@version: 0.1.3
          ...
          fix(0.1.3):改变之前cut后map两次的做法，直接根据format中的情况，（对于数值型变量）生成pd.cut需要用的bins和labels组成的字典，
                     （对于category类变量）生成map所需要的用的字典。
                     finebin中应该有最小值判断，甚至应该有智能的好坏数量判断？
                     加入多线程的池子
                     format定为左开右闭，还是左闭右开的问题，无法自由选择，因为pd.cut只能用一种，intervalIndex也只能用一种
